...
def dashboard(request):
    ...
    # ─── Employee's own attendance rate for current month ───
    employee_attendance_rate = 0
    employee_present_month = 0
    employee_working_days_month = 0

    if not user.is_superuser and not user.groups.filter(name='HR Admin').exists():
        # Employee's shift
        _emp_shift = None
        if hasattr(user, 'profile') and user.profile:
            _emp_shift = user.profile.shift

        # Working days elapsed this month (per shift)
        for _d in range(1, today.day + 1):
            _cur = date(today.year, today.month, _d)
            if _cur > today:
                break
            if _emp_shift:
                _attr = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'][_cur.weekday()]
                if not getattr(_emp_shift, _attr, False):
                    continue
            employee_working_days_month += 1

        # Present days this month
        employee_present_month = Attendance.objects.filter(
            user=user,
            date__year=today.year,
            date__month=today.month,
            status__in=['Present', 'Under Review', 'Missing Checkout',
                        'Weekend Work', 'Holiday Work']
        ).count()

        if employee_working_days_month > 0:
            employee_attendance_rate = round(
                    (employee_present_month / employee_working_days_month) * 100, 1
                )
...
def attendance_report(request):
    ...
    for emp in employees:
        ...
        # Skip employee if status filter is on and no records match
        if status_filter and status_filter != 'all' and not daily_records:
            continue
    ...
    if request.GET.get('download') == 'pdf':
        ...
        if download_type == 'single':
            ...
            pdf_employee_data = [
                ...
                if str(item['employee'].id) in emp_ids
            ]
            if not pdf_employee_data:
                messages.error(request, 'No matching employees found.')
                return redirect('attendance_report')
...
def employee_attendance_detail(request, user_id):
    from .utils import get_user_company, get_company_filtered, is_weekend_for_shift
    ...
    # ─── Attendance (company-filtered) — keyed by date ───
    attendances = get_company_filtered(
        request,
        Attendance.objects.filter(
            user=employee,
    ...
    leave_days = {}
    for leave in leaves:
        cur = max(leave.start_date, first_day)
    ...
    holidays = get_company_filtered(
        request,
        Holiday.objects.filter(date__gte=first_day, date__lte=last_day)
    )
    holiday_days = {h.date: h.name for h in holidays}
    ...
    max_day = last_day_num
    if year == today.year and month == today.month:
        max_day = today.day

    # Respect join date
    join_date = profile.date_of_joining if profile else None
    start_day = 1
    if join_date and join_date.year == year and join_date.month == month:
        start_day = join_date.day
    elif join_date and (join_date.year > year or (join_date.year == year and join_date.month > month)):
        start_day = max_day + 1
    ...
    for day in range(start_day, max_day + 1):
        ...
        if att:
            # ─── Derive status from times + workflow states ───
            if att.status in ('Under Review', 'On Leave', 'Incomplete'):
                status_label = att.status
            elif att.check_in_time and att.check_out_time:
                if _is_holiday:
                    status_label = 'Holiday Work'
                elif not _is_work_day:
                    status_label = 'Weekend Work'
                else:
                    status_label = 'Present'
            ...
            if current_date in leave_days and status_label == 'Absent':
                status_label = 'On Leave'

            # Format times
            if att.check_in_time:
                in_time = timezone.localtime(att.check_in_time).strftime('%I:%M %p')
            if att.check_out_time:
                out_time = timezone.localtime(att.check_out_time).strftime('%I:%M %p')
            ...
            # ─── Late / Early Out / Overtime — ONLY on working days ───
            if (att.check_in_time and att.check_out_time and shift
                ...
                if att.check_in_time > shift_start + timedelta(minutes=_grace):
                    ...
                    record_late_minutes = int(late_sec // 60)
            ...
            if status_label in ('Present', 'Late', 'Early Out', 'Under Review'):
                present += 1
            elif status_label == 'Missing Checkout':
                present += 1
                missing_checkout += 1
            elif status_label == 'Half Day':
                half_day += 1
            elif status_label == 'Weekend Work':
                weekend_work += 1
            elif status_label == 'Holiday Work':
                holiday_work += 1
            ...
            # ─── No attendance row ───
            if current_date in leave_days:
                status_label = 'On Leave'
            elif current_date in holiday_days:
    ...
    # ─── Working days (respects join date + holidays + shift weekends) ───
    start_counting_from = first_day
    if join_date and join_date > first_day:
        start_counting_from = join_date

    working_days = 0
    elapsed_working_days = 0
    for n in range(last_day_num):
        d = date(year, month, n + 1)
        if d < start_counting_from:
            continue
        if d in holiday_days:
            continue
        if is_weekend_for_shift(d, shift):
            continue
        working_days += 1
        if d <= today:
    ...
    context = {
        ...
        'working_days': elapsed_working_days,
        'late_days': late_days,
        'early_out_days': early_out_days,
...
def attendance_view(request):
    ...
    # ─── Employee's shift (for accurate weekend detection) ───
    employee_shift = None
    if hasattr(user, 'profile') and user.profile:
        employee_shift = user.profile.shift
    ...
    # Build per-day leave map: {date: True}
    leave_days = {}
    ...
    for day in range(1, last_day + 1):
        ...
        if att:
            ...
            # ─── No attendance row — check leave / weekend / future ───
            if current_date in leave_days:
                status = 'On Leave'
                check_in = check_out = None
                work_seconds = overtime_seconds = 0
            elif is_weekend_for_shift(current_date, employee_shift):
                status = 'Weekend'
                check_in = check_out = None
                work_seconds = overtime_seconds = 0
            elif current_date > today:
...
def leave_cancel(request, leave_id):
    ...
    if leave.status != 'Pending':
        messages.error(
            request,
            f'This leave request is already {leave.status} and cannot be cancelled.'
        )
        return redirect('employee_leaves')
    ...
    if request.method == 'POST':
        ...
        Notification.objects.filter(
            ...
            notification_type='info',
            message=(
                f"Leave request for {leave.start_date} – {leave.end_date} "
                f"was cancelled by {request.user.username}."
            ),
        )
    ...
    return render(request, 'leave_confirm_cancel.html', {'leave': leave})
...
def regularize_request(request):
    if request.method == 'POST':
        ...
        from .utils import get_category_usage
        ...
        category = RegularizationCategory.objects.filter(
            id=category_id, company=company, is_active=True
        ).first()
        ...
        usage = get_category_usage(
            request.user, company, category, reg_date_check.year, reg_date_check.month
        )
        if usage['remaining'] <= 0:
            messages.error(
                request,
                f"Monthly limit reached for {category.name}. "
                f"You have already used {usage['used']} of {usage['limit']} this month."
            )
            return redirect('regularize_request_list')
    ...
    categories = []
    if company:
        for cat in RegularizationCategory.objects.filter(
            company=company, is_active=True
        ).order_by('order', 'id'):
            info = get_category_usage(
                request.user, company, cat, today.year, today.month
            )
            categories.append({
                'obj':       cat,
                'used':      info['used'],
...
def payroll_generate(request):
    ...
    for salary in salaries:
        ...
        Payroll.objects.create(
            ...
            late_halfday_days=calc['late_halfday_days'],
...
def payslip_pdf(request, pk):
    ...
    # ─── Recompute present & split half-day (from attendance) ───
    ...
    half_day_units    = Decimal(str(half_day_count)) * Decimal('0.5')

    # Payroll.absent_days includes the half-day portion → strip it
    total_absent_days = Decimal(str(payroll.absent_days or 0))
    pure_absent_days  = max(Decimal('0'), total_absent_days - half_day_units)

    # Per-day rate
    working_days_dec = Decimal(str(payroll.working_days or 0))
    per_day = (
        Decimal(str(payroll.gross_salary)) / working_days_dec
        if working_days_dec > 0 else Decimal('0')
    )
    ...
    salary_data = [
        ...
        [
            ...
            Paragraph(f"Absence ({fmt_days(pure_absent_days)} day)", cell_style),
            Paragraph(f"Rs. {pure_absent_deduction:,.2f}", cell_amount_style),
        ],
        # HRA / Half-Day
        [
...
def payslip_view(request, pk):
    ...
    # ─── Compute Half-Day split (no model changes needed) ───
    ...
    # Payroll.absent_days already includes the half-day portion
    total_absent_days = Decimal(str(payroll.absent_days or 0))
    pure_absent_days  = max(Decimal('0'), total_absent_days - half_day_units)
...
def reg_category_create(request):
    ...
    if request.method == 'POST':
        ...
        if RegularizationCategory.objects.filter(company=company, name=name).exists():
...
@hr_admin_required
@company_required
def reg_category_delete(request, pk):
    from .utils import get_user_company
    from .models import RegularizationCategory
